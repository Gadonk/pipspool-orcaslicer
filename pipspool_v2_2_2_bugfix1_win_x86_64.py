...
PAGE_ICON_PATH = PLUGIN_DIR / "pipspool_page_icon.svg"
...
def desired_preset(
    ...
    # Always use a printer-neutral base. This repairs profiles that an older
    # PipSpool build accidentally attached to an unrelated printer preset.
    # Any compatibility chosen explicitly in Orca's Dependencies tab remains
    # stored in the existing preset and is preserved by this update.
...
def page_gate_data(spool: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if gate_value is None or gate_value < 0:
        return None
    data = page_spool_data(spool)
    data.update({"printer": printer or "Printer", "gate": int(gate_value)})
    return data
...
def page_gate_slots(gates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for gate in gates:
        grouped.setdefault(gate["printer"], {})[gate["gate"]] = gate
    slots = []
    for printer in sorted(grouped, key=str.casefold):
        ...
        for gate_number in range(slot_count):
            if gate_number in assigned:
                slots.append(assigned[gate_number])
            else:
                slots.append({
                    "printer": printer,
                    "gate": gate_number,
...
def pipspool_page_state(notice=None, friendly_initial_error=False) -> dict[str, Any]:
    ...
    gates = [gate for spool in spools if (gate := page_gate_data(spool))]
...
def pipspool_page_html(initial_state=None) -> str:
    ...
    for spool in initial_spools:
        ...
        spool_rows.append(
            ...
            f"<td>{escape(nozzle_text)}</td>"
            f"<td>{escape(bed_text)}</td>"
    ...
    for gate in initial_gates:
        ...
        loadout_cards.append(
            f'<article class="gate-tile{" empty-gate" if is_empty else ""}">'
            ...
            f'<span class="gate-remaining">{escape(remaining_text)}</span></article>'
        )
...
.report{{white-space:pre-wrap;background:#041326;border:1px solid #0d4f85;border-radius:8px;padding:11px;min-height:76px;color:#d7e7f4}}.toolbar{{display:flex;gap:9px;margin-bottom:10px}}
table{{width:100%;border-collapse:collapse}}th,td{{padding:9px 8px;border-bottom:1px solid #175486;text-align:left}}th{{color:var(--orca-muted);font-size:12px}}.swatch{{display:inline-block;width:14px;height:14px;border:1px solid #77a5c8;border-radius:50%;vertical-align:-2px;margin-right:7px}}
.gate-grid{{display:grid;grid-template-columns:repeat(8,minmax(108px,1fr));gap:10px;margin-top:14px}}.gate-tile{{min-width:0;min-height:184px;padding:11px 8px;border:1px solid #2385cf;border-radius:10px;background:linear-gradient(160deg,#0b3970,#061e3f);display:flex;flex-direction:column;align-items:center;text-align:center;box-shadow:0 5px 15px rgba(0,45,95,.28),inset 0 1px rgba(126,204,255,.1)}}.gate-status{{height:21px;color:#8fc8ff;font-size:11px}}.gate-image{{width:72px;height:58px;margin:3px 0 7px;display:block;filter:drop-shadow(0 4px 5px rgba(0,0,0,.32))}}.gate-material{{font-size:14px;line-height:1.2;color:var(--ivory)}}.gate-printer{{max-width:100%;margin-top:3px;color:var(--orca-muted);font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}.gate-remaining{{margin-top:auto;padding:3px 8px;border:1px solid #32a8eb;border-radius:10px;background:#08345a;color:var(--cyan);font-size:12px;font-weight:650}}.empty-gate{{opacity:.58}}.empty-gate .gate-status{{color:#d3d6da;font-size:13px;font-weight:700}}.empty-gate .gate-remaining{{visibility:hidden}}
.spool-wrap{{max-height:420px;overflow:auto}}.wide{{grid-column:1/-1}}details{{border-top:1px solid var(--orca-border);padding:11px 0}}summary{{cursor:pointer;font-weight:650}}.count{{float:right;color:var(--orca-muted);font-weight:400}}
.group-actions{{display:flex;gap:7px;margin:10px 0}}.group-actions button{{padding:4px 9px;font-size:12px}}.choices{{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:6px 14px}}.choice{{display:flex;align-items:center;gap:8px;min-height:25px}}input[type=checkbox]{{accent-color:var(--orca-accent)}}
...
function renderSpools(){{
  ...
  for(i=0;i<state.spools.length;i++){{
    ...
    var values=['#'+spool.id,spool.material,spool.name,spool.vendor,'',spool.nozzle_temperature==null?'—':spool.nozzle_temperature+' °C',spool.bed_temperature==null?'—':spool.bed_temperature+' °C',spool.remaining==null?'—':spool.remaining+' g'];
...
function renderLoadout(gates){{
  ...
  for(i=0;i<gates.length;i++){{
    ...
    var tile=document.createElement('article');tile.className='gate-tile'+(empty?' empty-gate':'');
    ...
    var remaining=document.createElement('span');remaining.className='gate-remaining';remaining.textContent=empty?'':(gate.remaining==null?'— g':gate.remaining+' g');
    tile.appendChild(status);tile.appendChild(image);tile.appendChild(material);tile.appendChild(printer);tile.appendChild(remaining);body.appendChild(tile);
  }}
