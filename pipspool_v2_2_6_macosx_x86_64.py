...
class SpoolmanClient:
    ...
    def _validated_field_value_migrations(
        ...
        response = requests.get(f"{self.base_url}/api/v1/filament", timeout=10)
        response.raise_for_status()
    ...
    def _replace_field_definition_safely(
        ...
        try:
            ...
            response.raise_for_status()
            deleted = True
            response = requests.post(endpoint, json=desired, timeout=10)
            response.raise_for_status()
            ...
            try:
                ...
                if getattr(response, "status_code", 200) != 404:
                    response.raise_for_status()
                response = requests.post(endpoint, json=original, timeout=10)
                response.raise_for_status()
...
def decoded_orca_overrides(
    ...
    for setting in selected_settings:
        ...
        if setting in ORCA_LIST_TEXT_SETTINGS and isinstance(value, list):
            result[setting] = [str(item) for item in value if item is not None]
            continue
    ...
    for filament_id, filament_spools in grouped.items():
        ...
        for preset_name in live_names:
            if preset_name in live_preset_values:
                master.update(live_preset_values[preset_name])
        extras = dict(filament.get("extra") or {})
        ...
        for directory, files_by_id in zip(directories, indexed):
            ...
            try:
                ...
                display_name, desired = desired_preset(
                    spool, raw, profiles, selected_settings
                )
                target = directory / f"{safe_filename(display_name)}.json"
...
.connection-layout{{display:grid;grid-template-columns:minmax(270px,330px) minmax(0,1fr);gap:20px;align-items:start}}.connection-side{{min-width:0}}.gate-panel{{min-width:0;padding-left:20px;border-left:1px solid var(--orca-border)}}.gate-panel h2{{margin-bottom:2px;color:var(--ivory)}}
...
@media(max-width:1100px){{.connection-layout{{grid-template-columns:1fr}}.gate-panel{{padding:17px 0 0;border-left:0;border-top:1px solid var(--orca-border)}}.gate-grid{{grid-template-columns:repeat(4,minmax(108px,1fr))}}}}@media(max-width:850px){{.grid{{grid-template-columns:1fr}}.top-actions{{margin-left:0}}header{{flex-wrap:wrap}}.gate-grid{{grid-template-columns:repeat(2,minmax(108px,1fr))}}}}
