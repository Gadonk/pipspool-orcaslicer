...
DEFAULT_SPOOL_TABLE_COLUMNS = SPOOL_TABLE_COLUMNS[:-1]
...
def pipspool_page_html(initial_state=None) -> str:
    ...
    for spool in initial_spools:
        ...
        spool_rows.append(
            ...
            f'<td data-column="remaining">{escape(remaining_text)}</td>'
            f'<td data-column="location">{escape(str(spool.get("location") or "—"))}</td></tr>'
        )
    ...
    for group, fields in ORCA_SETTING_GROUPS.items():
        ...
        field_sections.append(
            f'<details class="field-group"><summary>{escape(group)} <span class="count" data-group="{escape(group)}">{group_count} selected</span></summary>'
...
<section class="card"><h2>Active spools</h2><div class="toolbar"><input id="search" type="search" placeholder="Search by spool number, material, colour, manufacturer, name or location"><details class="columns-menu"><summary>Columns</summary><div class="column-choices">{''.join(f'<label><input type="checkbox" data-table-column="{column}"{" checked" if column in initial_table_columns else ""}>{column.title()}</label>' for column in SPOOL_TABLE_COLUMNS)}</div></details></div><div class="spool-wrap"><table><thead><tr><th>Spool</th><th>Material</th><th data-column="filament">Filament</th><th data-column="manufacturer">Manufacturer</th><th data-column="colour">Colour</th><th data-column="nozzle">Nozzle</th><th data-column="bed">Bed</th><th data-column="remaining">Remaining</th><th data-column="location">Location</th></tr></thead><tbody id="spools">{spool_rows_html}</tbody></table></div><p id="empty" class="muted"{" hidden" if initial_spools else ""}>No matching active spools.</p></section>
...
function updateCounts(){{
  var sections=document.querySelectorAll('.field-group');
  var i;
...
function renderSpools(){{
  ...
  for(i=0;i<state.spools.length;i++){{
    ...
    var columns=['','', 'filament','manufacturer','colour','nozzle','bed','remaining','location'];
    var values=['#'+spool.id,spool.material,spool.name,spool.vendor,'',spool.nozzle_temperature==null?'—':spool.nozzle_temperature+' °C',spool.bed_temperature==null?'—':spool.bed_temperature+' °C',spool.remaining==null?'—':spool.remaining+' g',spool.location||'—'];
