...
# These belong exclusively to OrcaSlicer's Dependencies tab. PipSpool must not
# create, infer or overwrite them; user-selected compatibility is authoritative.
    ...
    def parent_for(self, vendor: str, material: str) -> str:
        ...
        for name in matching:
            if name.strip().casefold() == exact_generic:
                return name
        for name in matching:
            if name.casefold().startswith(f"generic {parent_type}"):
                return name
        # Never select the first arbitrary match: system presets are commonly
        # tied to a particular printer and Orca then hides the generated preset
        # when a different printer is active.
...
def desired_preset(
    ...
    dependency_values = {
        key: preset[key] for key in ORCA_DEPENDENCY_KEYS if key in preset
    }
    ...
    # Preserve deliberate restrictions configured in Orca's Dependencies tab.
    # New PipSpool profiles remain unrestricted because no dependency is added.
...
def pipspool_page_state(notice=None, friendly_initial_error=False) -> dict[str, Any]:
    ...
    try:
        statuses = profile_sync_statuses(spools, orca_profiles(), selected_fields)
    except Exception as exc:
...
var state={{spools:[],busy:false,lowStockThreshold:{initial_low_stock_threshold}}};
